...
function Segments() {
  ...
  const handleDateFiltreChange = (e) => {
  ...
  return (
    <section className="page-section city-page">
      <div className="city-panel">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              <label style={labelStyle}>
                Train disponible{" "}
                {form.date && <span style={{ color: "var(--accent-blue)" }}>({loadingTrains ? "..." : `${trainsDisponibles.length} dispo`})</span>}
              </label>
        ...
        {/* Filtre liste par date */}
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "16px" }}>
          <label style={{ ...labelStyle, whiteSpace: "nowrap" }}>Afficher les segments du</label>
          <input type="date" value={dateFiltre} onChange={handleDateFiltreChange} style={{ ...inputStyle, width: "auto" }} />
          {dateFiltre && (
            <span style={{ color: "var(--text-muted)", fontSize: "0.85rem", whiteSpace: "nowrap" }}>
              {loadingSegments ? "Chargement..." : `${segments.length} segment(s)`}
            </span>
          )}
        </div>
